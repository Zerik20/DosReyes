<div id="footer" class="p-5 bg-center" style="background-image: url(img/bg-footer.jpg);">
    <div class="container mt-4">
        
        <div class="border-bottom">
            <h4 class="text-center text-white oro">Mezcal Artesanal</h4>
            <h2 class="text-center text-white">Dos Reyes</h2>
            <p class="text-center text-white">CALIDAD EN CADA GOTA</p>
        </div>
        <div class="mt-5 d-flex justify-content-center menu-footer">
            <a class="nav-link mx-5 text-white active" aria-current="page" href="#">NOSOTROS</a>
            <a class="nav-link mx-5 text-white" href="#">PRODUCTOS</a>
            <!--a class="nav-link mx-5 text-white" href="#">Mix & Food</!--a-->
            <a class="nav-link mx-5 text-white" href="#">CONTACTO</a>
        </div>
    </div>
   <div class="iconos text-center d-flex justify-content-center my-5">
        <a href="#" class="text-decoration-none">
            <div class="cont-icon-footer mx-3">
                <i class="fas fa-envelope"></i>
            </div>    
        </a>
        
        <a href="#" class="text-decoration-none">
            <div class="cont-icon-footer mx-3">
                <i class="fas fa-phone-volume"></i>
            </div>    
        </a>
        
        <a href="#https://wa.me/qr/A2KUZGHE2QEVN1" class="text-decoration-none">
            <div class="cont-icon-footer mx-3">
                <i class="fab fa-whatsapp"></i>
            </div>    
        </a>
        
        <a href="#" class="text-decoration-none">
            <div class="cont-icon-footer mx-3">
                <i class="fab fa-facebook-f"></i>
            </div>    
        </a>
        
    </div>
    <p class="text-white text-center">©2024 Dos Reyes; Todos los derechos reservados.</p>
</div>